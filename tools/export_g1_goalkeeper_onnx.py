"""Export G1 goalkeeper checkpoint to ONNX via PolicyOnnx wrapper.

Usage:
    python tools/export_g1_goalkeeper_onnx.py \
        --checkpoint legged_gym/resources/weight/goalkeeper_official.pt \
        --output legged_gym/logs/g1/exported/policies/goalkeeper_official.onnx
"""
...
def parse_args():
    ...
    parser.add_argument("--checkpoint", type=str,
                        default="legged_gym/resources/weight/goalkeeper_official.pt")
    parser.add_argument("--output", type=str,
                        default="legged_gym/logs/g1/exported/policies/goalkeeper_official.onnx")
...
def main():
    ...
    # Build ActorCritic with G1 goalkeeper dimensions
    # num_actor_obs=960, num_critic_obs=113, num_one_step_obs=96, actor_history_length=10, num_actions=29
    ...
    # Verify forward pass
    dummy_input = torch.zeros(1, 960, dtype=torch.float32)
    with torch.no_grad():
        output = policy_onnx(dummy_input)
    print(f"Dummy inference: input {dummy_input.shape} -> output {output.shape}")
    assert output.shape == (1, 29), f"Expected (1, 29), got {output.shape}"
